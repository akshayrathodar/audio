...
export default class AudioUploadAr {
  ...
  render(){
    ...
    var mainwrapperdiv = this._make('div','modal-dialog',{'role':'document',"id":"mainmodel","data-id":"rootmodel","data-modelid":this.modelId});
    ...
    if(this.data.audiolink != '' && this.data.audiolink != undefined) {
        ...
        s.style.width = '100%';
        s.id  = 'audioplayer';
        s.setAttribute('data-name',this.data.audioname);
        s.load();
        ...
        s.addEventListener("canplay", function() {
          ...
          //textb.type = "text";
          textb.className = "cdx-input";
          //textb.placeholder = "Ok me ";
          //textb.value = this.data.audioname;
          textb.setAttribute('style','text-align:left');
          textb.innerHTML = audioname;
          var divboth = document.createElement('div');
          divboth.appendChild(textb);
          divboth.appendChild(s);

          ip.appendChild(divboth);
           //ip.appendChild(s);
     ...
     h1loading.innerHTML = 'Audio is Loading... ';

     secondcenter.appendChild(h1loading); //green
     var h1loader      = this._make('span',['cdx-loader','hide'],{"id":"loadercls"});
     secondcenter.appendChild(h1loader); //green
     ...
     let imagelinkId = this.ID();


     var liuploadtab    = this._make('li','active',{'role':'presentation'});
     var uploadinga     = this._make('a','',{"href":"#uploadimg"+uploadimgid,"aria-controls":"uploadimg","role":"tab","data-toggle":"tab","aria-expanded":"true"});
     ...
     var linkattacha    = this._make('a','',{"href":"#imglinkeditor"+imagelinkId,"aria-controls":"imglinkeditor","role":"tab","data-toggle":"tab","aria-expanded":"false"});
    ...
    var divtabpanel     = this._make('div',['tab-pane','active'],{"role":"tabpanel","id":"uploadimg"+uploadimgid});
    var divchoosupload  = this._make('div',['choose-upload','text-center'],null);
    var divmainbtnsmbtn = this._make('div',['main-btn','sm-btn'],null);

    var inputeditorfileupload = this._make('input','',{"type":"file","name":"editoraudioupload","id":"editoraudioupload"});
    ...
    var formateDiv   = this._make('small','','');
     formateDiv.innerHTML = '.mp3, .WAV, .ogg format will be allowed';
     divchoosupload.appendChild(formateDiv); // green
     
    // #####################
    divtabpanel.appendChild(divchoosupload);
    // #####################

    var divtabpanelimglinkeditor = this._make('div','tab-pane',{"role":"tabpanel","id":"imglinkeditor"+imagelinkId});
    var divchoosupload2 = this._make('div',["choose-upload","text-center"],null);
    
    var divformgroupcf = this._make('div',['form-group','cf'],null);
    var inputeditorlinkpast = this._make('input','is-form-control',{"type":"text","name":"editorlinkpast","id":"editorlinkpast","placeholder":"Paste an Audio Link.."});
    ...
    // // ####################################
     ...
     inputeditorfileupload.addEventListener('change',(event)=>{
        ...
        const validImageTypes = ['audio/mpeg', 'audio/x-wav', 'video/ogg','audio/wav','audio/ogg'];
        if (validImageTypes.includes(fileType)) {
            //remove error msg
              $('#edload').removeClass('hide');
              $('#loadercls').removeClass('hide');
            ...
            // end remove error msg
              ...
              $.ajax({
                ...
                {
                  $('#edload').addClass('hide');
                  $('#loadercls').addClass('hide');
                  
                  if(result.file == "largefile") { 
                     smalleditorerrormsg.style.color = "RED";
                     smalleditorerrormsg.innerHTML = "Please Upload Small Audio Or Upgrade Your Plan";
                     $(event.target).parents('.modal-content').removeClass('hide');
                    //$('#editorerrormsg').html('Please Upload Small Audio Or Upgrade Your Plan');
                  } else {

                  var s = document.createElement("audio");
                  s.setAttribute('controls','controls');
                  s.setAttribute('data-name',result.body.filename);
                  s.src = result.body.file.url;
                  s.style.width = '100%';
                  s.id  = '_' + Math.random().toString(36).substr(2, 9);
                  s.load();
                  $('#hiddenaudio').val(result.body.file.url);
                  s.addEventListener("canplay", function() {
                    smalleditorerrormsg.innerHTML = '';
                    ...
                    textb.className = "cdx-input";                    
                    textb.setAttribute('style','text-align:left');
                    textb.innerHTML = result.body.filename;
                    var divboth = document.createElement('div');
                    divboth.className = 'cdx-button';
                    divboth.appendChild(textb);
                    divboth.appendChild(s);
                    $(event.target).parents('.modal-dialog').html(divboth);
                    //$(event.target).parents('.modal-dialog').html(s);

                    this.tempLink = result.body.file.url;
                    this.save();
                  });
      ...
      buttoneditorlinkbtn.addEventListener("click",(event)=>{
          var link = inputeditorlinkpast.value;
           ...
           if (link)
           {
            ...
            s.id  = '_' + Math.random().toString(36).substr(2, 9);
            s.load();
            s.setAttribute('data-name',resfile);
            s.addEventListener("canplay", function() {
  ...
  save(blockContent){
    ...
    return {"audiolink":$(blockContent).find('audio').attr('src'),'audioname':$(blockContent).find('audio').attr('data-name')};
  }
   ...
   removed() {
    ...
    if(result.includes(this.config.domain))
    {
        ...
        fd.append('pg_identification',$('#pg_identification').val());
         ...
         $.ajax({
          url: "http://localhost/workspace/ws/v1.0/editor.deattachaudio", 
          type: "POST",
          data: fd,
          processData: false,
          contentType: false,
